    ...
    public class RealWorldTerrainEasyRoads3DGenerator: RealWorldTerrainRoadGenerator
    {
        ...
        private static void CreateTCross(ERConnection[] connections, List<DelayedConnectionItem> items, RealWorldTerrainOSMNode node, Vector3 wp, int[] indices)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                float a = RealWorldTerrainMath.Angle2D(p1, wp, p2, false);
                if (Mathf.Abs(a) < Mathf.Abs(ma))
                {
                    ma = a;
                    angleY = RealWorldTerrainMath.Angle2D(p1, p2);
                    ...
                    float ca = RealWorldTerrainMath.Angle2D(p1, wp, secondPoints[j], false);
                ...
                for (int j = i + 1; j < 4; j++)
                {
                    ...
                    if (Mathf.Abs(a) < Mathf.Abs(ma))
                    {
                        ...
                        for (int k = 0; k < 4; k++)
                        {
                            ...
                            p2 = secondPoints[k];
                            a = RealWorldTerrainMath.Angle2D(p1, wp, p2, false);
                            if (a < 0) indices[k] = 2;
                            else indices[k] = 3;
                        }
                    }
        ...
        public static void Init()
        {
            ...
            ERModularBase erModularBase = RealWorldTerrainUtils.FindObjectOfType<ERModularBase>();
            ...
            if (prefs.sizeType == 2)
            {
                ...
                double sizeZ = RealWorldTerrainGeo.EARTH_RADIUS * Math.Acos(scfY * sctY + ccfY * cctY);
